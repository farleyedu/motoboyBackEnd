    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            try
            {
                ...
                // Ler o arquivo SQL
                var scriptPath = Path.Combine(Directory.GetCurrentDirectory(), "..", "dados_ficticios.sql");
                ...
                foreach (var command in commands)
                {
                    ...
                    // Pular comentários, linhas vazias e comandos não executáveis
                    if (string.IsNullOrWhiteSpace(trimmedCommand) || 
                        trimmedCommand.StartsWith("--") || 
                        trimmedCommand.StartsWith("/*") ||
                        trimmedCommand.ToUpper().StartsWith("PRINT") ||
                        trimmedCommand.ToUpper().StartsWith("SELECT") && !trimmedCommand.ToUpper().Contains("SETVAL"))
                    {
                        continue;
                    }
