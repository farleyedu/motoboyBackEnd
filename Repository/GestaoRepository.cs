...
SET tipo_acesso              = EXCLUDED.tipo_acesso,
    ...
    updated_at               = NOW();";
                ...
                var establishmentRole = command.EstablishmentRole ?? "atendente";
                foreach (var estabelecimentoId in command.EstabelecimentoIds)
                {
                    await connection.ExecuteAsync(sqlUsuarioEstabelecimento, new
                    {
                        ...
                        UserId = userId,
                        EstabelecimentoId = estabelecimentoId,
                        TipoAcesso = establishmentRole,
                        command.ActorUserId,
                        PermissoesCustomizadas = command.PermissoesCustomizadasJson
                    }, transaction);

                    if (string.Equals(establishmentRole, "motoboy", StringComparison.OrdinalIgnoreCase))
                    {
