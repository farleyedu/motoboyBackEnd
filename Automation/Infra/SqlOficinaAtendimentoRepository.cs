...
{
    public class SqlOficinaAtendimentoRepository : IOficinaAtendimentoRepository
    {
        ...
        public SqlOficinaAtendimentoRepository(IConfiguration configuration)
        {
        ...
        public async Task<OficinaAtendimento?> ObterAbertoAsync(Guid idEstabelecimento, string telefoneE164)
        {
...
SELECT id,
       ...
       perfil_contato     AS PerfilContato,
 ...
 LIMIT 1;";
            ...
            var row = await connection.QueryFirstOrDefaultAsync<OficinaAtendimentoRow>(sql, new
            {
        ...
        public async Task<OficinaAtendimento?> ObterPorConversaAsync(Guid idConversa)
        {
...
SELECT id,
 ...
 LIMIT 1;";
            ...
            await using var connection = new NpgsqlConnection(_connectionString);
            var row = await connection.QueryFirstOrDefaultAsync<OficinaAtendimentoRow>(sql, new { IdConversa = idConversa });
            return Map(row);
        }

        public async Task<Guid> CriarAsync(OficinaAtendimento atendimento)
        {
...
INSERT INTO cliente_oficina (
    ...
    @DataAtualizacao
        ...
        public async Task AtualizarAsync(OficinaAtendimento atendimento)
        {
            ArgumentNullException.ThrowIfNull(atendimento);
...
CREATE TABLE IF NOT EXISTS cliente_oficina (
    ...
    perfil_contato VARCHAR(30) NOT NULL DEFAULT 'indefinido',
    intencao_principal VARCHAR(60) NOT NULL DEFAULT 'duvida_geral',
...
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS nome_cliente VARCHAR(160);
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS perfil_contato VARCHAR(30) NOT NULL DEFAULT 'indefinido';
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS intencao_principal VARCHAR(60) NOT NULL DEFAULT 'duvida_geral';
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS intencao_detalhe VARCHAR(60);
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS resumo_atendimento TEXT;
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS status VARCHAR(60) NOT NULL DEFAULT 'em_triagem';
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS etapa_atual VARCHAR(120);
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS ultima_pergunta TEXT;
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS dados_extras JSONB NOT NULL DEFAULT '{}'::jsonb;
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS via_numero_central BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS data_handover TIMESTAMPTZ NULL;
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS data_conclusao TIMESTAMPTZ NULL;
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS data_criacao TIMESTAMPTZ NOT NULL DEFAULT NOW();
ALTER TABLE cliente_oficina ADD COLUMN IF NOT EXISTS data_atualizacao TIMESTAMPTZ NOT NULL DEFAULT NOW();

UPDATE cliente_oficina
   SET dados_extras = '{}'::jsonb
 WHERE dados_extras IS NULL;

CREATE INDEX IF NOT EXISTS ix_cliente_oficina_conversa
    ON cliente_oficina (id_conversa);

CREATE INDEX IF NOT EXISTS ix_cliente_oficina_cliente
    ON cliente_oficina (id_cliente);

CREATE INDEX IF NOT EXISTS ix_cliente_oficina_estab_status_updated
    ON cliente_oficina (id_estabelecimento, status, data_atualizacao DESC);

CREATE INDEX IF NOT EXISTS ix_cliente_oficina_estab_tel_aberto
    ON cliente_oficina (id_estabelecimento, telefone_e164, data_atualizacao DESC);";
        ...
        private static object ToParameters(OficinaAtendimento atendimento)
        {
            return new
            {
                atendimento.Id,
        ...
        private static OficinaAtendimento? Map(OficinaAtendimentoRow? row)
        {
            if (row == null)
            {
                return null;
            ...
            return new OficinaAtendimento
            {
                ...
                PerfilContato = string.IsNullOrWhiteSpace(row.PerfilContato) ? "indefinido" : row.PerfilContato!,
                IntencaoPrincipal = string.IsNullOrWhiteSpace(row.IntencaoPrincipal) ? "duvida_geral" : row.IntencaoPrincipal!,
        ...
        private static string SerializeExtras(IReadOnlyDictionary<string, object?>? extras)
        {
            return JsonSerializer.Serialize(extras ?? new Dictionary<string, object?>(), JsonOptions);
        }
        ...
        private sealed class OficinaAtendimentoRow
        {
            ...
            public string? PerfilContato { get; set; }
