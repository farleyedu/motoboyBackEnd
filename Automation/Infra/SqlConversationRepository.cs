...
UPDATE conversas c
 ...
 WHERE c.id_cliente = cl.id
   ...
   AND (@PreservarConversaId IS NULL OR c.id <> @PreservarConversaId);",
        ...
        private Task<IEnumerable<ConversationMessageItemDto>> ExactMessagesAsync(NpgsqlConnection cx, Guid id, DateTime? before, int limit) => cx.QueryAsync<ConversationMessageItemDto>(@"SELECT m.id AS Id, COALESCE(m.criada_por, '') AS CriadaPor, COALESCE(m.conteudo, '') AS Conteudo, COALESCE(m.tipo::text, 'texto') AS Tipo, COALESCE(m.status::text, '') AS Status, m.data_envio AS DataEnvio, COALESCE(m.data_criacao, m.data_envio, NOW()) AS DataCriacao FROM mensagens m WHERE m.id_conversa = @Id AND (@Before::timestamptz IS NULL OR COALESCE(m.data_criacao, m.data_envio) < @Before::timestamptz) ORDER BY COALESCE(m.data_criacao, m.data_envio) DESC LIMIT @Limit;", new { Id = id, Before = before, Limit = limit });
        private Task<IEnumerable<ConversationMessageItemDto>> GroupMessagesAsync(NpgsqlConnection cx, Guid groupId, DateTime? before, int limit) => cx.QueryAsync<ConversationMessageItemDto>(@"SELECT m.id AS Id, COALESCE(m.criada_por, '') AS CriadaPor, COALESCE(m.conteudo, '') AS Conteudo, COALESCE(m.tipo::text, 'texto') AS Tipo, COALESCE(m.status::text, '') AS Status, m.data_envio AS DataEnvio, COALESCE(m.data_criacao, m.data_envio, NOW()) AS DataCriacao FROM mensagens m JOIN conversas c ON c.id = m.id_conversa WHERE COALESCE(c.id_conversa_grupo, c.id) = @GroupId AND (@Before::timestamptz IS NULL OR COALESCE(m.data_criacao, m.data_envio) < @Before::timestamptz) ORDER BY COALESCE(m.data_criacao, m.data_envio) DESC LIMIT @Limit;", new { GroupId = groupId, Before = before, Limit = limit });
        private Task<IEnumerable<ConversationMessageItemDto>> ClientMessagesAsync(NpgsqlConnection cx, Guid idCliente, Guid idEstabelecimento, DateTime? before, int limit) => cx.QueryAsync<ConversationMessageItemDto>(@"SELECT m.id AS Id, COALESCE(m.criada_por, '') AS CriadaPor, COALESCE(m.conteudo, '') AS Conteudo, COALESCE(m.tipo::text, 'texto') AS Tipo, COALESCE(m.status::text, '') AS Status, m.data_envio AS DataEnvio, COALESCE(m.data_criacao, m.data_envio, NOW()) AS DataCriacao FROM mensagens m JOIN conversas c ON c.id = m.id_conversa WHERE c.id_cliente = @IdCliente AND c.id_estabelecimento = @IdEstabelecimento AND (@Before::timestamptz IS NULL OR COALESCE(m.data_criacao, m.data_envio) < @Before::timestamptz) ORDER BY COALESCE(m.data_criacao, m.data_envio) DESC LIMIT @Limit;", new { IdCliente = idCliente, IdEstabelecimento = idEstabelecimento, Before = before, Limit = limit });
